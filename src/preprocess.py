...
def _filter_errors(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if removed_count > 0:
        logger.info(
            f"Отфильтровано {removed_count} некорректных записей (было {initial_count}, стало {filtered_count})"
        )
...
def _convert_dates(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if invalid_dates_count > 0:
        logger.warning(
            f"Найдено {invalid_dates_count} записей с невалидными датами, они будут удалены"
        )
...
def _merge_student_data(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if "student_id" not in df.columns:
        logger.warning(
            "Столбец 'student_id' не найден, пропуск объединения по студентам"
        )
        return df
    ...
    if len(available_keys) >= 2:  # Минимум 2 ключевых поля для проверки дубликатов
        ...
        if duplicates_removed > 0:
            logger.info(
                f"Удалено {duplicates_removed} дубликатов при объединении данных по студентам"
            )
